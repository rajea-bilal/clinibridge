...
Don't fetch or derive app state in useEffect; reserve effects for real external effects only (DOM, subscriptions, analytics)
Fetch on mount/param change → route loader or framework data fetching pattern
Submit/mutate → server function or mutation handler → then invalidate queries/router
Sync UI ↔ querystring → typed search params + navigation helpers
Derived state → compute during render (`useMemo` only if expensive)
Subscribe external stores → `useSyncExternalStore` (expect hydration fallbacks during SSR)
DOM/listeners/widgets → small `useEffect`/`useLayoutEffect`
Compute derived state during render; `useMemo` only if expensive
Keep page/UI state in the URL with typed search params when appropriate
Server-synced domain data → use data fetching libraries or framework loaders
Ephemeral UI/session → state management library; do not mirror server data into client state management
Derived views → compute in render or via live queries
Use `useActionState` for form pending/error/result state
Use `use` hook to suspend on promises
Wrap sync updates that might suspend during hydration with `startTransition` (direct import from 'react')
...
`useSyncExternalStore` always triggers fallbacks during hydration
Use `useSyncExternalStore(subscribe, getSnapshot)` for external store subscriptions (expect hydration fallbacks)