...
function Header({
  ...
  return (
    <div
      className={cn(
        "relative mb-4 rounded-none border border-white/10 bg-white/5 p-4",
        className
      )}
      {...props}
    >
...
function Description({ className, ...props }: React.ComponentProps<"p">) {
  return (
    <p
      className={cn("font-mono text-white/60 text-xs", className)}
      {...props}
    />
  );
}
...
function PlanName({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      className={cn(
        "flex items-center gap-2 font-medium font-mono text-sm text-white/60 [&_svg:not([class*='size-'])]:size-4",
        className
      )}
      {...props}
    />
...
function Badge({ className, ...props }: React.ComponentProps<"span">) {
  return (
    <span
      className={cn(
        "rounded-full border border-white/10 px-2 py-0.5 font-mono text-white/70 text-xs",
        className
      )}
      {...props}
    />
...
function MainPrice({ className, ...props }: React.ComponentProps<"span">) {
  return (
    <span
      className={cn(
        "font-extrabold font-mono text-3xl text-white tracking-tight",
        className
      )}
      {...props}
    />
...
function Period({ className, ...props }: React.ComponentProps<"span">) {
  return (
    <span
      className={cn("pb-1 font-mono text-sm text-white/60", className)}
      {...props}
    />
  );
}
...
function OriginalPrice({ className, ...props }: React.ComponentProps<"span">) {
  return (
    <span
      className={cn(
        "mr-1 ml-auto font-mono text-lg text-white/40 line-through",
        className
      )}
      {...props}
    />
...
function ListItem({ className, ...props }: React.ComponentProps<"li">) {
  return (
    <li
      className={cn(
        "flex items-start gap-3 font-mono text-sm text-white/80",
        className
      )}
      {...props}
    />
...
function Separator({
  ...
  return (
    <div
      className={cn(
        "flex items-center gap-3 font-mono text-sm text-white/60",
        className
      )}
      {...props}
    >
      <span className="h-[1px] flex-1 bg-white/20" />
      <span className="shrink-0 text-white/60">{children}</span>
      <span className="h-[1px] flex-1 bg-white/20" />
    </div>
  );
}
